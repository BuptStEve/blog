...
这样在使用组件库时，可以直接 import 用到的组件，随后在打包时会自动将没有用到的组件代码“摇掉”。
...
<img :src="$withBase('/imgs/ui-lib/js-modules-via-script-tag.png')" alt="js-modules-via-script-tag">
...
但其实除非你用的是 webpack v1，不然现在构建工具基本都支持 tree-shaking。
...
想要了解更多可以看看 webpack 文档：https://webpack.docschina.org/guides/tree-shaking/
...
### 小结
本文从使用组件库的两种方式，引入各种构建产物类型和如何实现 js 和样式的按需加载。

接着说明了应该用 rollup 作为组件库的构建工具，并给出项目文件结构的示例和 package.json 中相关字段的说明。
