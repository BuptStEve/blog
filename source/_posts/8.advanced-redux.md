...
乐观更新比较直观的栗子就是在微信、QQ等通讯软件中，发送的消息立即在对话窗口中展示，如果发送失败了，在消息旁边展示提示即可。由于这种交互中“乐观”地相信操作会成功，因此称作乐观更新。
...
redux-thunk 的思路是保持 action 和 reducer 简单纯粹，然鹅 side effects（在前端主要体现在异步操作上）的复杂度是不可避免的，因此它将其放在了 action creator 步骤，通过 thunk 函数手动控制每一次的 dispatch。
